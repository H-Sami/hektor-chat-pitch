...
| The hosted MCP endpoint `mcp.slidev-mcp.org:443` refuses TCP connections while `slidev-mcp.org` (same IP `188.213.129.204`) serves normally | `render_slides` / `export_slides` / `screenshot_slides` cannot run at all | Deck verified through the local Slidev path instead: local build, per-slide screenshots, and an 18-page PDF export. Re-run the MCP publish once the service answers |
...
7. Are there contractual constraints on sending customer data to a third-party model provider?
